<template>
  <nav
    class="fixed top-0 left-0 right-0 flex justify-between items-center p-4 bg-transparent transition-all ease-in-out duration-300 shadow-lg"
    :class="{ 'bg-white dark:bg-gray-900': isScrolled }"
  >
    <NuxtLink
      to="/"
      class="flex items-center gap-2 text-2xl font-bold text-blue-500"
    >
    ...
    <div class="flex space-x-1 lg:space-x-6">
      <ClientOnly v-if="isAuthenticated">
        <UButton @click="logout" color="red"> Deslogar </UButton>
      </ClientOnly>
